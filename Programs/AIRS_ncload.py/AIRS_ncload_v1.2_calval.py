...
if proj == 'lcc':
	...
	meridians_y = np.full(len(meridians_x), -40)
	meridian_labels_x, meridian_labels_y = pyproj.transform(pcyl, plcc, meridians_x, meridians_y)
	for i in range(len(meridians_x)):
		plt.annotate(str(abs(meridians_x[i]))+"$\!^\circ\!$"+"W", xy=(meridian_labels_x[i], meridian_labels_y[i]), xytext=(meridian_labels_x[i]-250000, meridian_labels_y[i]+175000), zorder = 5, fontsize=14)
	...
	for i in range(len(parallels_y)):
		plt.annotate(str(abs(parallels_y[i]))+"$\!^\circ\!$"+"S", xy=(parallel_labels_x[i], parallel_labels_y[i]), xytext=(parallel_labels_x[i]-750000, parallel_labels_y[i]-85000), zorder = 5, fontsize=14)
...
# Saving figure
plt.savefig('testAIRS2.png',dpi=600)
print("testAIRS2.png saved in: ", os.getcwd())
