...
---v1.6.1-Control_testing [Azores Box]----------------------------------
...
for file in os.listdir(directory):
	...
	"""=============================================================="""
	"""====Test to see if orbit is sufficiently within Azores box===="""
	"""=============================================================="""
	...
	box = np.where(data_lat<25, 0, (np.where(data_lat>50, 0,
		(np.where(data_lon>340, 0, (np.where(data_lon<300, 0, 1)))))))
	...
	for u in grouped_diffs:
		if u[0] != 0: # Bypass 1's and -1's
			...
			if data_lat[itrn] < 50 and data_lat[itrn] > 25 and \
			box[itrn] == 1:
		...
		if pc_or_im == 'pc':
			...
			# ~ z2 = savgol_filter(z, 15, 2, axis = 1) # Vertical S-G filter
			z = z1-z2
		...
		map = Basemap(projection='cyl',llcrnrlat=10,urcrnrlat=70,\
					llcrnrlon=-80,urcrnrlon=0,resolution='i', ax=ax2)
