...
for file in os.listdir(directory):
	...
	for u in grouped_diffs:
		...
		print(myear, mmonth, mday, mtime)
		mdoy = yyyymmdd_to_doy(myear, mmonth, mday)
		ncfile = ERA5_dir + myear + '/era5_' + myear + 'd' \
		+ mdoy + '.nc'
		print('ERA5 netCDF file:')
		print(ncfile, '\n')
		"""data = ERA5_dataload(ncfile)
		...
		np.where(ERA5_data_lat)
		lat_range = np.where(ERA5_data_lat<-80, 0, (np.where(ERA5_data_lat>-40, 0, 1)))
		lon_range = np.where(ERA5_data_lon<-80, 0, (np.where(ERA5_data_lon>-40, 0, 1)))
		print(lat_range)
		for lat in range(len(lat_range)):
			if lat_range[lat] == 1:
		...
		if pc_or_im == 'pc':
			...
			# ~ z = data_obs - data_mod # Find perturbations relative to unsmoothed ERA5
			z = data_mod
			# ~ z = data_obs
			...
			topography = ax1.fill_between(x_topo, y_topo/1000, -10, color= '#ffcc66')
