...
if imshow == True:
	...
	if interp == False:
		...
		cs = plt.contourf(xi2,yi2/1000,zi2, cmap='RdBu_r',
			levels=np.linspace(-20, 20, 41))
			
	try:
		cs4 = ax1.contour(xi2, yi2/1000, zi2, levels=[-30,-25,-20,-15,-10,-5,5,10,15,20,25,30], linewidths = 0.4, colors='k', vmin=-30, vmax=30, alpha=1)
	except:
		print("Unable to plot contours")
	else:
		cls = ax1.clabel(cs4, [-30,-25,-20,-15,-10,-5,5,10,15,20,25,30], fmt = '%i', fontsize=4, inline_spacing=1)
	try:		
...
mask = plt.contourf(x, y/1000, hatcharray, cmap = grayhatchescmap, zorder = 2)
...
date_form = dates.DateFormatter('%b') # Sets date format #T
ax1.xaxis.set_major_formatter(date_form) #T
...
ax1.set_yticks(np.arange(31)) #T
ax1.yaxis.set_major_locator(plt.MaxNLocator(16)) #T
ax1.yaxis.set_minor_locator(plt.MaxNLocator(31)) #T
